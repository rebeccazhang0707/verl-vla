...
@hydra.main(config_path="config", config_name="main_sac", version_base=None)
def main(config):
...
def main_task(config):
    ...
    try:
        ...
        return {
            ...
            "resource_pool_spec": cluster.resource_pool_spec,
            "worker_groups": {
                name: {"world_size": int(worker_group.world_size)}
                for name, worker_group in cluster.worker_groups.items()
            },
        }
    finally:
        cluster.shutdown()
