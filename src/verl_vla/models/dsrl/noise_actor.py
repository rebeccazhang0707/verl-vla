...
"""DSRL noise actor: a Transformer chunking SAC policy over flow-matching noise.
...
``x0`` (DSRL, arXiv:2506.15799).

The architecture mirrors the posttrain reference DSRL actor
...
sinusoidal positional encoding, and mixed by a Transformer encoder so every
flow step gets its own tanh-squashed Gaussian latent while sharing parameters.
The tanh change-of-variables correction uses the numerically exact softplus
form the reference DSRL stack relies on.
...
embedding and normalized state. ``noise_per_step`` selects an independent latent
per horizon step (GR00T/posttrain parity) or one shared latent broadcast over
the chunk (pi0 RLinf parity, a single-token transformer).
"""
