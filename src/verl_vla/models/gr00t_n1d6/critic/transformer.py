...
The action tokens are the steering noise ``full_action`` (the SAC action space
under DSRL). The backend keeps the exact interface of :class:`Gr00tCriticGroup`
(``forward`` cat/min, target network, polyak update, parameter enumeration) so
the SAC training worker is unchanged.
"""
...
class Gr00tTransformerCriticGroup(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # Which state-features key supplies the observation token: GR00T's mean
        # pooled VL prefix ("pooled") or, with the DINOv2 option, "dino_embedding".
        self.feature_key = "pooled"
    ...
    def forward(
        ...
        *,
        ...
        pooled = state_features[self.feature_key]
