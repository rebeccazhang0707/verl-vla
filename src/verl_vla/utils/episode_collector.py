...
class EpisodeCollector:
    ...
    def metrics(self) -> dict[str, float]:
        ...
        return {
            "data/collector_episodes_flushed": float(self._episodes_flushed),
            "data/collector_success_episodes_flushed": float(self._success_episodes_flushed),
            "data/collector_forced_flushes": float(self._forced_flushes),
            "data/collector_transitions_emitted": float(self._transitions_emitted),
            "data/collector_slots_dropped": float(self._slots_dropped),
            "data/collector_open_len_max": float(max(open_lens, default=0)),
            "data/collector_open_len_mean": float(np.mean(open_lens)) if open_lens else 0.0,
        }
