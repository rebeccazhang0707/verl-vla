...
#   * model/override@…=gr00t     -> GR00T SAC override (policy_type=arena, …)
...
"$PYTHON" -m verl_vla.entrypoints.train.sac \
  ...
  "cluster.actor_rollout_ref.model.override_config.embodiment_tag=$GROOT_EMBODIMENT_TAG" \
  "cluster.actor_rollout_ref.model.override_config.embodiment_id=$GROOT_EMBODIMENT_ID" \
  "cluster.actor_rollout_ref.model.override_config.action_dim=$ACTION_DIM" \
  "cluster.actor_rollout_ref.model.override_config.num_action_chunks=$NUM_ACTION_CHUNKS" \
  "cluster.actor_rollout_ref.model.override_config.critic_action_horizon=$NUM_ACTION_CHUNKS" \
